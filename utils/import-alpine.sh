...
get_rootfs_tar() {
    ...
    for arch in $alpine_archs; do
        mkdir -p "${DIR}/alpine/tars/$alpine_version/$arch"
	...
	local filename="alpine-minirootfs-${alpine}-${arch}.tar.gz"
        ...
        if ! wget -c -L -O \
            "${DIR}/alpine/tars/${alpine_version}/$arch/alpine.tar.gz" "$rootfs_url"; then
            echo "Failed to download rootfs file version ${alpine_version} for arch ${arch}"
	    return 1
        fi

        # Grab the signature for the file
        if ! wget -c -L -O \
            "${DIR}/alpine/tars/${alpine_version}/$arch/key.asc" "${rootfs_url}.asc"; then
            echo "Failed to download signature file version ${alpine_version} for arch ${arch}"
	    return 1
        fi

        # Validate the downloaded root file system
        if ! gpg --verify "${DIR}/alpine/tars/${alpine_version}/$arch/key.asc"\
            "${DIR}/alpine/tars/${alpine_version}/$arch/alpine.tar.gz"; then
